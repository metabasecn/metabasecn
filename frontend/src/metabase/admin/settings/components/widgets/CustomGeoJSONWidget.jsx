...
const EditMap = ({ map, onMapChange, originalMap, geoJson, geoJsonLoading, geoJsonError, onLoadGeoJson, onCancel, onSave }) =>
    <div className="flex">
        <div className="flex-no-shrink">
            <h2>{ !originalMap ? "添加地图" : "编辑地图" }</h2>
            <SettingContainer description="地图名">
                <div className="flex">
                    <input
                        type="text"
                        className="SettingsInput AdminInput bordered rounded h3"
                        placeholder="举例：北京，天津"
                        value={map.name}
                        onChange={(e) => onMapChange({ ...map, "name": e.target.value })}
                    />
                </div>
            </SettingContainer>
            <SettingContainer description="地图json文件地址">
                <div className="flex">
                    <input
                        type="text"
                        className="SettingsInput AdminInput bordered rounded h3"
                        placeholder="举例 https://my-mb-server.com/maps/my-map.json"
                        value={map.url}
            ...
            <div className={cx({ "disabled": !geoJson })}>
                <SettingContainer description="哪个属性指定区域的标识符？">
                ...
                <SettingContainer description="哪个属性指定区域的显示名?">
                    <GeoJsonPropertySelect
