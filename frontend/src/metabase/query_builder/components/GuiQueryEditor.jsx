    ...
    renderFilters() {
        ...
        if (this.props.tableMetadata) {
            ...
            if (Query.canAddFilter(this.props.datasetQuery.query)) {
                addFilterButton = this.renderAdd((filterList ? null : "添加查询筛选条件"), null, "addFilterTarget");
            }
        } else {
            enabled = false;
            addFilterButton = this.renderAdd("添加查询筛选条件", null, "addFilterTarget");
        }
            ...
            for (let i = 0; i < breakouts.length; i++) {
                ...
                breakoutList.push(
                    <BreakoutWidget
                        ...
                        addButton={this.renderAdd(i === 0 ? "设置数据分组条件" : null)}
                    />
                );
    ...
    renderGroupedBySection() {
        ...
        return (
            <div className="GuiBuilder-section GuiBuilder-groupedBy flex align-center px1" ref="viewSection">
                <span className="GuiBuilder-section-label Query-label">分组条件</span>
